...
class Project extends Base
{
    ...
    public function addProject()
    {
        if(request()->isPost()){
            ...
            $flag = $article->insertProject($param);
    ...
    public function editProject()
    {
        ...
        if(request()->isPost()){
            $param = input('post.');
            $flag = $article->editProject($param);
            return json(['code' => $flag['code'], 'data' => $flag['data'], 'msg' => $flag['msg']]);
        }
        $id = input('param.id');
        $data = $article->getOneProject($id);
        $map[] = ['status','=',1];
     ...
     * [articlePic 模板图片]
     */
    public function projectIcon($id){
        $article = new ProjectModel();
        if(request()->isPost()){
            $param = input('post.');
            $flag = $article->editProject($param);
        ...
        $pic['height'] = 310;
        $this->assign('pic',$pic);
        return $this->fetch('project_thum_pic');
    }
    /**
     * [uplaodPic 上传图片]
     */
    ...
    public function projectState()
    {
        extract(input());
        $article = new ProjectModel();
        $flag = $article->ProjectState($id,$num);
        return json(['code' => $flag['code'], 'data' => $flag['data'], 'msg' => $flag['msg']]);
    }
}