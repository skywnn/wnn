...
use app\home\model\MyInfoModel;
...
class MyInfo extends Base
{
    public function myInfoIndex(){
        ...
        $lists = $article->getMyInfoByWhere($map, $Nowpage, $limits,$od);
    ...
    public function getOneAddress(){
        $celebrity = new MyInfoModel();
        $res = $celebrity->getOneLaw(input('get.id'));
        $this->assign('item',$res);
        return $this->fetch('address/address_show');
    }
}