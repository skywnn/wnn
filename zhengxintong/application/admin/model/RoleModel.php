...
//        }
    ...
    {
        $roleIds = is_array($role_id) ? [] : [(int)$role_id];
        return Db::name('role')->where('id', 'in', $roleIds)->column('rules');
    }
    ...
    {
        $data = Db::name('menu')->where(array('status'=>1))->order(['sort' => 'asc', 'create_time' => 'asc'])->select();
        return $data ? $data : [];
    }
}