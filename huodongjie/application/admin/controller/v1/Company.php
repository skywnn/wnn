...
use app\admin\model\CompanyTrendsModel;
use app\admin\model\TradeModel;
use app\admin\model\ThirdDevconfigModel;
use app\admin\model\TrendsModel;
            ...
            if (!empty($data)) {
                ...
                foreach ($data as $k=>$v) {
                    switch ($k) {
                        case 'logo':
                            $logo = $uplaod
                                ->picArrayUplaod($v,'Company_logo',true,uniqid(),400,400);
                            $param['logo'] = $logo['data'];
                            break;
                        case 'pic':
                            $pic = $uplaod
                                ->picArrayUplaod($v,'Company_pic',true,uniqid(),400,400);
                            $param['pic'] = $pic['data'];
                            break;
                    }
                }
            ...
            if (!empty($data)) {
                ...
                foreach ($data as $k=>$v) {
                    switch ($k) {
                        case 'logo':
                            $logo = $uplaod
                                ->picArrayUplaod($v,'Company_logo',true,$param['id'],400,400);
                            $param['logo'] = $logo['data'];
                            if (!empty($param['id'])) {
                                $item = Db::name('company')->where('id',$param['id'])->find();
                                $oldurl=$item['logo'];
                                @unlink('static'.$oldurl);
                            }
                            break;
                        case 'pic':
                            $pic = $uplaod
                                ->picArrayUplaod($v,'Company_pic',true,$param['id'],400,400);
                            $param['pic'] = $pic['data'];
                            if (!empty($param['id'])) {
                                $item = Db::name('company')->where('id',$param['id'])->find();
                                $oldurl=$item['pic'];
    ...
    public function trendsIndex(){
        if(request()->isAjax ()){
            ...
            if($field && $order){
                ...
                $od="ct.create_time desc";
            }
            $map[] = ['ct.company_id','=',input('param.id')];
            $company = new CompanyModel();
            $nowpage = input('page') ? input('page'):1;
            $limits = input("limit")?input("limit"):10;
            $count = Db::name('company_trends')->alias('ct')->where($map)->count();
            $lists = Db::name('company_trends ct')
                ->join('company c','c.id=ct.company_id')
                ->join('member m','m.id=ct.member_id')
                ->where($map)->field('ct.*,m.nick_name')->select();
            return json(['code'=>220,'msg'=>'','count'=>$count,'data'=>$lists]);
        }
        $id = input('param.id');
        $item = Db::name('company')->where('id',$id)->find();
        $this->assign('item',$item);
